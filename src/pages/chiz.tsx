...
export default function ChizBox() {
    ...
    const { ratBalance, cheddazBalance, chizBalance, ratContract, dispenserContract, dispenserAmountClaimed, rewardPerRat } = useContracts()
    ...
    const claim = async () => {
        try {
            ...
            setStatus('idle')
            await dispenserContract.methods.megaClaimChiz().estimateGas({ from: wallet.account })
            await dispenserContract.methods.megaClaimChiz().send({ from: wallet.account })
            setStatus('complete')
        } catch (error) {
            setStatus('error')
                ...
                <motion.div initial={{ y: '100%' }} animate={{ y: '0%' }} transition={{ type: 'tween', duration: 0.3 }} className="border-emboss bg-yellow-500 p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            ...
                            <div className="border-deboss bg-black text-white p-2">
                                <p className="text-2xl">{cheddazBalance || '??'}</p>
                                <p className="text-xs">Cheddaz</p>
                            </div>
                            <div className="hidden sm:block border-deboss bg-black text-white p-2">
                                <p className="text-2xl">{chizBalance ? `~${NumberSuffix.format(Number(chizBalance))}` : '??'}</p>
                                <p className="text-xs">CHIZ</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative border-deboss bg-yellow-900 p-4 text-white">
                        ...
                        <div className="mb-4">
                            <p className="font-extrabold">Claimable Rewards</p>
